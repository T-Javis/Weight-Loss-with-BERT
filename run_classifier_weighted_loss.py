...
flags.DEFINE_bool(
    "weighted_loss", False,
    "Use weighted loss to solve unbalanced data")
...
class WeightLossTextProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    file_path=os.path.join(data_dir,'dev_data.txt') 
    f=open(file_path,'r',encoding="utf-8")
    dev_data=[]
    index=0
    for line in f.readlines():
  ...
  def get_weight(self, data_dir):
    ...
    for line in f.readlines():
      ...
      #print(line)
      index+=1
      if label not in num_dic:
        num_dic[label]=0
      else:
        num_dic[label]+=1
    print('-------------num_dic',num_dic)
    for key,value in num_dic.items():
      num_dic[key]=1/(value/index)
    
    label_list=self.get_labels()
    for label in label_list:
      weight.append(num_dic[label])
    print('weight-----',weight)
    return weight
  ...
  with tf.variable_scope("loss"):
    if is_training:
      ...
      if FLAGS.weighted_loss:
        print('\n6'*10)
        weight_ones=tf.Variable(tf.ones([tf.shape(labels)[0],num_labels]), name='weight',dtype=tf.float32)
        alpha=weight_ones*weight_list
        per_example_loss = -tf.reduce_sum((one_hot_labels * log_probs * alpha), axis=-1)
      else:
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, num_labels):
        ...
        pr=tf.multiply(tf.to_float(precision), tf.to_float(recall))
        pr2=tf.multiply(tf.to_float(2), tf.to_float(pr))
        p_r=tf.add(tf.to_float(precision),tf.to_float(recall))
        f1=tf.divide(pr2, p_r)
        return {
            "eval_accuracy": accuracy,
            "eval_loss": loss,
            "recall": recall,
...
def main(_):
  ...
  processors = {
      ...
      "mrpc": MrpcProcessor,
      "xnli": XnliProcessor,      
      'weightedloss' : WeightLossTextProcessor,
  }
  ...
  weight_list=processor.get_weight(FLAGS.data_dir)
